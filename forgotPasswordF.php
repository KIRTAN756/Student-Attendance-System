	...
	<table border="1" class="table1">
        ...
        <?php
			...
			if (isset($_POST['find'])) {
				...
				$result = mysqli_query($con, "SELECT F_Password FROM faculty_info WHERE F_Name = '$fullName' AND F_Gender = '$gender' AND F_Email = '$mail' AND F_MobileNo = '$mono' AND Security_Question = '$question' AND Security_Answer = '$answer' ");

				while ($res = mysqli_fetch_array($result)) {
					if($res){
                        $_SESSION['status']="Password: ".$res['F_Password'];
